...
#include <uecm/crypto/impl/openssl_init.h>
...
static bool alloc_mutexes() {
    ...
    #if defined(_WIN32) || defined(_WIN64)
    	uecm_safe_alloc(mutex_buf, CRITICAL_SECTION, total);
    #else
    	uecm_safe_alloc(mutex_buf, pthread_mutex_t, total);
    #endif
