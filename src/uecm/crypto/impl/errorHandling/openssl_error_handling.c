...
char *uecm_openssl_error_handling_impl(char *begin_msg) {
	...
	if (error_buffer) {
		error_buffer = uecm_strcat_variadic("ssss", begin_msg, " - failed with error msg `", error_buffer, "`");
	} else {
		error_buffer = uecm_strcat_variadic("ssl", begin_msg, " - failed with error code ", error_code);
	}
