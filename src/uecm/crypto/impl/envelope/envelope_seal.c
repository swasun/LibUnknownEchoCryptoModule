...
bool envelope_seal_buffer(EVP_PKEY *pub_key, unsigned char *plaintext, int plaintext_len,
	...
	*iv_len = EVP_CIPHER_iv_length(cipher);
    uecm_safe_alloc_or_goto(*iv, unsigned char, *iv_len, clean_up);

	uecm_safe_alloc_or_goto(*encrypted_key, unsigned char, EVP_PKEY_size(pub_key), clean_up);

	uecm_safe_alloc_or_goto(*ciphertext, unsigned char, plaintext_len + *iv_len, clean_up);
