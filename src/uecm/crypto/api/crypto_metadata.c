...
#include <uecm/crypto/utils/friendly_name.h>
#include <ei/ei.h>
...
static bool crypto_metadata_write_asym(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid,
...
static bool crypto_metadata_read_asym(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid,
...
uecm_crypto_metadata *uecm_crypto_metadata_create_empty() {
    ...
    uecm_safe_alloc(metadata, uecm_crypto_metadata, 1);
...
void uecm_crypto_metadata_destroy(uecm_crypto_metadata *metadata) {
    if (metadata) {
        uecm_safe_free(metadata->cipher_name);
        uecm_safe_free(metadata->digest_name);
        uecm_safe_free(metadata);
    }
}
...
bool uecm_crypto_metadata_set_cipher_certificate(uecm_crypto_metadata *metadata, uecm_x509_certificate *certificate) {
    ...
    if (!(metadata->cipher_pk = uecm_rsa_public_key_from_x509_certificate(certificate))) {
...
bool uecm_crypto_metadata_set_signer_certificate(uecm_crypto_metadata *metadata, uecm_x509_certificate *certificate) {
    ...
    if (!(metadata->signer_pk = uecm_rsa_public_key_from_x509_certificate(certificate))) {
        ei_stacktrace_push_msg("Failed to extract RSA public key from specified certificate");
        return false;
    }
...
bool uecm_crypto_metadata_certificates_exists(const char *folder_name, const char *uid) {
    ...
    if (!uecm_is_dir_exists(folder_name)) {
        return false;
    }
    ...
    if (!(cipher_certificate_file_name = uecm_strcat_variadic("ssss", folder_name, "/", uid, "_CIPHER.pem"))) {
        ei_stacktrace_push_msg("Failed to build cipher certificate file name");
        return false;
    }

    if (!(signer_certificate_file_name = uecm_strcat_variadic("ssss", folder_name, "/", uid, "_SIGNER.pem"))) {
        ei_stacktrace_push_msg("Failed to build signer certificate file name");
        goto clean_up;
    }

    if (uecm_is_file_exists(cipher_certificate_file_name) ||
        uecm_is_file_exists(signer_certificate_file_name)) {
        result = true;
    } else {
        result = false;
    }
...
clean_up:
    uecm_safe_free(cipher_certificate_file_name);
    uecm_safe_free(signer_certificate_file_name);
    return result;
}
...
bool uecm_crypto_metadata_exists(const char *folder_name, const char *uid) {
    ...
    if (!(sym_file_name = uecm_strcat_variadic("sss", folder_name, "/", uid, "_sym"))) {
        ei_stacktrace_push_msg("Failed to build file name for sym crypto metadata");
        goto clean_up;
    }

    if (!(asym_cipher_file_name = uecm_strcat_variadic("ssss", folder_name, "/", uid, "_asym_CIPHER"))) {
        ei_stacktrace_push_msg("Failed to build file name with name '%s' for keystore 'CIPHER'", uid);
        goto clean_up;
    }

    if (!(asym_signer_file_name = uecm_strcat_variadic("ssss", folder_name, "/", uid, "_asym_SIGNER"))) {
        ei_stacktrace_push_msg("Failed to build file name with name '%s' for keystore 'SIGNER'", uid);
        goto clean_up;
    }

    if (uecm_is_file_exists(sym_file_name) ||
        uecm_is_file_exists(asym_cipher_file_name) ||
        uecm_is_file_exists(asym_signer_file_name)) {
...
clean_up:
    uecm_safe_free(sym_file_name);
    uecm_safe_free(asym_cipher_file_name);
    uecm_safe_free(asym_signer_file_name);
    return result;
}
...
bool uecm_crypto_metadata_write_certificates(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid) {
    ...
    if (!uecm_is_dir_exists(folder_name)) {
        ...
        if (!uecm_create_folder(folder_name)) {
...
clean_up:
    uecm_safe_free(cipher_certificate_file_name);
    uecm_safe_free(signer_certificate_file_name);
    uecm_safe_fclose(cipher_certificate_fd);
    uecm_safe_fclose(signer_certificate_fd);
    return result;
}
...
bool uecm_crypto_metadata_read_certificates(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid) {
    ...
    if (!uecm_is_file_exists(signer_certificate_file_name)) {
...
static bool crypto_metadata_write_sym(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid, const char *password) {
    ...
    if (!(sym_encrypter = uecm_sym_encrypter_default_create(uecm_sym_key_create_from_string(password)))) {
        ei_stacktrace_push_msg("Failed to ");
        goto clean_up;
    }

    if (!(file_name = uecm_strcat_variadic("ssss", folder_name, "/", uid, "_sym"))) {
        ei_stacktrace_push_msg("Failed to build file name for sym crypto metadata");
        goto clean_up;
    }

    if (metadata->sym_key) {
        uecm_byte_writer_append_int(stream, (int)metadata->sym_key->size);
    } else {
        uecm_byte_writer_append_int(stream, 0);
    }

    if (metadata->digest_name) {
        uecm_byte_writer_append_int(stream, (int)strlen(metadata->digest_name));
    } else {
        uecm_byte_writer_append_int(stream, 0);
    }

    if (metadata->cipher_name) {
        uecm_byte_writer_append_int(stream, (int)strlen(metadata->cipher_name));
    } else {
        uecm_byte_writer_append_int(stream, 0);
    }

    if (metadata->sym_key) {
        uecm_byte_writer_append_bytes(stream, metadata->sym_key->data, metadata->sym_key->size);
    }

    if (metadata->digest_name) {
        uecm_byte_writer_append_string(stream, metadata->digest_name);
    }

    if (metadata->cipher_name) {
        uecm_byte_writer_append_string(stream, metadata->cipher_name);
    }

    uecm_safe_alloc(iv, unsigned char, iv_size);
    ...
    if (!uecm_sym_encrypter_encrypt(sym_encrypter, uecm_byte_stream_get_data(stream), uecm_byte_stream_get_size(stream),
    ...
    uecm_byte_stream_clean_up(stream);
    uecm_byte_stream_set_position(stream, 0);
    if (!uecm_byte_writer_append_int(stream, iv_size)) {
        ei_stacktrace_push_msg("Failed to write iv size to stream");
        goto clean_up;
    }
    if (!uecm_byte_writer_append_bytes(stream, iv, iv_size)) {
        ei_stacktrace_push_msg("Failed to write iv to stream");
        goto clean_up;
    }
    if (!uecm_byte_writer_append_int(stream, (int)cipher_data_size)) {
        ei_stacktrace_push_msg("Failed to write cipher data size to stream");
        goto clean_up;
    }
    if (!uecm_byte_writer_append_bytes(stream, cipher_data, cipher_data_size)) {
        ei_stacktrace_push_msg("Failed to write cipher data to stream");
        goto clean_up;
    }

    if (!uecm_write_binary_file(file_name, uecm_byte_stream_get_data(stream), uecm_byte_stream_get_size(stream))) {
...
static bool crypto_metadata_write_asym(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid,
    ...
    if (!(friendly_name = uecm_friendly_name_build((unsigned char *)uid, strlen(uid), keystore_type, &friendly_name_size))) {
        ei_stacktrace_push_msg("Failed to build friendly name for '%s'", keystore_type);
        goto clean_up;
    }

    if (!(string_friendly_name = uecm_string_create_from_bytes(friendly_name, friendly_name_size))) {
        ei_stacktrace_push_msg("Failed to convert friendly name from bytes to string");
        goto clean_up;
    }

    if (!(keystore = uecm_pkcs12_keystore_create(certificate, private_key, string_friendly_name))) {
        ei_stacktrace_push_msg("Failed to create pkcs12 keystore with friendly name '%s'", string_friendly_name);
        goto clean_up;
    }

    if (!(file_name = uecm_strcat_variadic("sssss", folder_name, "/", uid, "_asym_", keystore_type))) {
...
clean_up:
    ...
    uecm_safe_free(friendly_name);
    uecm_safe_free(file_name);
    uecm_safe_free(string_friendly_name);
    return result;
}
...
static bool crypto_metadata_read_sym(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid,
    ...
    bool result;
    const char *file_name;
    uecm_byte_stream *raw_data_stream, *iv_stream, *cipher_data_stream, *plain_data_stream;
    ...
    result = false;
    file_name = NULL;
    raw_data_stream = uecm_byte_stream_create();
    iv_stream = uecm_byte_stream_create();
    cipher_data_stream = uecm_byte_stream_create();
    plain_data_stream = uecm_byte_stream_create();
    raw_data = NULL;
    plain_data = NULL;
    sym_encrypter = NULL;
    sym_key_data = NULL;
    ...
    if (!(raw_data = uecm_read_binary_file(file_name, &raw_data_size))) {
        ei_stacktrace_push_msg("Failed to read binary file '%s'", file_name);
        goto clean_up;
    }

    if (!uecm_byte_writer_append_bytes(raw_data_stream, raw_data, raw_data_size)) {
        ei_stacktrace_push_msg("Failed to copy raw data to byte stream");
        goto clean_up;
    }
    uecm_byte_stream_set_position(raw_data_stream, 0);

    if (!uecm_byte_read_next_stream(raw_data_stream, iv_stream)) {
        ei_stacktrace_push_msg("Failed to read and copy IV to stream");
        goto clean_up;
    }
    if (!uecm_byte_read_next_stream(raw_data_stream, cipher_data_stream)) {
    ...
    if (!uecm_sym_encrypter_decrypt(sym_encrypter, uecm_byte_stream_get_data(cipher_data_stream), uecm_byte_stream_get_size(cipher_data_stream),
        uecm_byte_stream_get_data(iv_stream), &plain_data, &plain_data_size)) {
        ei_stacktrace_push_msg("Failed to decrypt cipher data");
        goto clean_up;
    }

    if (!uecm_byte_writer_append_bytes(plain_data_stream, plain_data, plain_data_size)) {
        ei_stacktrace_push_msg("Failed to copy plain data to stream");
        goto clean_up;
    }
    uecm_byte_stream_set_position(plain_data_stream, 0);

    if (!uecm_byte_read_next_int(plain_data_stream, &sym_key_size)) {
        ei_stacktrace_push_msg("Failed to read sym key size");
        goto clean_up;
    }
    if (!uecm_byte_read_next_int(plain_data_stream, &digest_name_size)) {
        ei_stacktrace_push_msg("Failed to read digest name size");
        goto clean_up;
    }
    if (!uecm_byte_read_next_int(plain_data_stream, &cipher_name_size)) {
        ei_stacktrace_push_msg("Failed to read cipher name size");
        goto clean_up;
    }

    if (sym_key_size > 0) {
        if (!uecm_byte_read_next_bytes(plain_data_stream, &sym_key_data, sym_key_size)) {
    ...
    if (digest_name_size > 0) {
        if (!uecm_byte_read_next_string(plain_data_stream, &digest_name, (size_t)digest_name_size)) {
    ...
    if (cipher_name_size > 0) {
        if (!uecm_byte_read_next_string(plain_data_stream, &cipher_name, (size_t)cipher_name_size)) {
...
clean_up:
    uecm_safe_free(file_name);
    uecm_byte_stream_destroy(raw_data_stream);
    uecm_byte_stream_destroy(iv_stream);
    uecm_byte_stream_destroy(cipher_data_stream);
    uecm_byte_stream_destroy(plain_data_stream);
    uecm_safe_free(raw_data);
...
static bool crypto_metadata_read_asym(uecm_crypto_metadata *metadata, const char *folder_name, const char *uid,
    ...
    if (!(keystore = uecm_pkcs12_keystore_load(file_name, password))) {
    ...
    if (!(*public_key = uecm_rsa_public_key_from_x509_certificate(*certificate))) {
        ei_stacktrace_push_msg("Failed to extract RSA public key from specified certificate");
