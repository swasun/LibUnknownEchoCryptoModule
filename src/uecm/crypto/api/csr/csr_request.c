...
unsigned char *uecm_csr_build_client_request(uecm_x509_certificate *certificate, uecm_private_key *private_key,
    ...
    if (!uecm_byte_writer_append_int(stream, (int)strlen(csr_string))) {
		ei_stacktrace_push_msg("Failed to write CSR string size to stream");
		goto clean_up;
	}
    if (!uecm_byte_writer_append_int(stream, (int)future_key->size)) {
		ei_stacktrace_push_msg("Failed to write future key size to stream");
		goto clean_up;
	}
    if (!uecm_byte_writer_append_int(stream, (int)iv_size)) {
		ei_stacktrace_push_msg("Failed to write IV size to stream");
		goto clean_up;
	}
    if (!uecm_byte_writer_append_string(stream, csr_string)) {
		ei_stacktrace_push_msg("Failed to write CSR string to stream");
		goto clean_up;
	}
    if (!uecm_byte_writer_append_bytes(stream, future_key->data, future_key->size)) {
    ...
    if (!uecm_cipher_plain_data(uecm_byte_stream_get_data(stream), uecm_byte_stream_get_size(stream), ca_public_key, NULL, &cipher_data,
...
clean_up:
    ...
    uecm_safe_free(signed_certificate_buffer);
...
unsigned char *uecm_csr_build_server_response(uecm_private_key *csr_private_key, uecm_x509_certificate *ca_certificate, uecm_private_key *ca_private_key,
    ...
    if (!uecm_byte_writer_append_bytes(stream, decipher_data, decipher_data_size)) {
		ei_stacktrace_push_msg("Failed to write deciphered client CSR");
		goto clean_up;
	}
	uecm_byte_stream_set_position(stream, 0);

    uecm_byte_read_next_int(stream, &read_int);
    if (read_int == 0) {
    ...
    if (!(uecm_byte_read_next_bytes(stream, &decipher_client_request, decipher_client_request_size))) {
        ei_stacktrace_push_msg("Failed to read decipher client request");
        goto clean_up;
    }

    if (!(uecm_byte_read_next_bytes(stream, &key_data, key_size))) {
        ei_stacktrace_push_msg("Failed to read asym key to use");
        goto clean_up;
    }

    if (!(uecm_byte_read_next_bytes(stream, &iv, iv_size))) {
...
clean_up:
    ...
    uecm_safe_free(decipher_client_request);
    uecm_safe_free(iv);
    uecm_byte_stream_destroy(stream);
    uecm_sym_key_destroy(key);
    uecm_safe_free(key_data);
    uecm_sym_encrypter_destroy(sym_encrypter);
    uecm_x509_csr_destroy(csr);
    uecm_safe_free(string_pem_certificate);
