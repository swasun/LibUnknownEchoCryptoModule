/******************************************************************************************
 * Copyright (C) 2018 by Charly Lamothe													  *
 *																						  *
 * This file is part of LibUnknownEchoCryptoModule.										  *
 *																						  *
 *   LibUnknownEchoCryptoModule is free software: you can redistribute it and/or modify   *
 *   it under the terms of the GNU General Public License as published by				  *
 *   the Free Software Foundation, either version 3 of the License, or					  *
 *   (at your option) any later version.												  *
 *																						  *
 *   LibUnknownEchoCryptoModule is distributed in the hope that it will be useful,        *
 *   but WITHOUT ANY WARRANTY; without even the implied warranty of						  *
 *   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the						  *
 *   GNU General Public License for more details.										  *
 *																						  *
 *   You should have received a copy of the GNU General Public License					  *
 *   along with LibUnknownEchoCryptoModule.  If not, see <http://www.gnu.org/licenses/>.  *
 ******************************************************************************************/

#include <uecm/crypto/api/cipher/data_cipher.h>
#include <uecm/crypto/api/signature/signer.h>
#include <uecm/crypto/api/encryption/sym_encrypter.h>
#include <uecm/crypto/api/compression/compress.h>
#include <uecm/crypto/utils/crypto_random.h>
#include <uecm/crypto/factory/sym_encrypter_factory.h>
#include <uecm/crypto/factory/rsa_signer_factory.h>
#include <uecm/crypto/factory/sym_key_factory.h>
#include <uecm/crypto/impl/envelope/envelope_seal.h>
#include <uecm/crypto/impl/envelope/envelope_open.h>
#include <uecm/byte/byte_stream.h>
#include <uecm/byte/byte_writer.h>
#include <uecm/byte/byte_reader.h>
#include <uecm/byte/byte_utility.h>
#include <uecm/alloc.h>
#include <ei/ei.h>

bool uecm_cipher_plain_data(unsigned char *plain_data, size_t plain_data_size,
    uecm_public_key *public_key, uecm_private_key *private_key,
    unsigned char **cipher_data, size_t *cipher_data_size, const char *cipher_name,
    const char *digest_name) {

    bool result;
    unsigned char *encrypted_key, *iv, *cipher_data_temp, *signature, *compressed;
    int encrypted_key_len, iv_len, cipher_data_len_temp;
    uecm_byte_stream *stream;
    size_t signature_size, compressed_size;
    uecm_signer *signer;

    result = false;
    encrypted_key = NULL;
    iv = NULL;
    cipher_data_temp = NULL;
    stream = uecm_byte_stream_create();
    signature = NULL;
    signature_size = 0;
    signer = NULL;
    compressed = NULL;

    if (!(compressed = uecm_compress_buf(plain_data, plain_data_size, &compressed_size))) {
		ei_stacktrace_push_msg("Failed to compress ReceiverHeader content");
		goto clean_up;
	}

    if (!envelope_seal_buffer(uecm_public_key_get_impl(public_key), compressed,
        (int)compressed_size, &encrypted_key, &encrypted_key_len, &iv, &iv_len,
    	&cipher_data_temp, &cipher_data_len_temp, cipher_name)) {

        ei_stacktrace_push_msg("Failed to envelope buffer");
        goto clean_up;
    }

    if (private_key) {
        if (!(signer = uecm_rsa_signer_create(public_key, private_key, digest_name))) {
            ei_stacktrace_push_msg("Failed to create rsa uecm_signer with key pair");
            goto clean_up;
        }

        if (!uecm_signer_sign_buffer(signer, plain_data, plain_data_size, &signature, &signature_size)) {
            ei_stacktrace_push_msg("Failed to sign message with our private key");
            goto clean_up;
        }
    }

    uecm_byte_writer_append_int(stream, encrypted_key_len);
    uecm_byte_writer_append_int(stream, iv_len);
    uecm_byte_writer_append_int(stream, cipher_data_len_temp);
    uecm_byte_writer_append_int(stream, (int)signature_size);
    uecm_byte_writer_append_int(stream, (int)plain_data_size);
    uecm_byte_writer_append_bytes(stream, encrypted_key, (size_t)encrypted_key_len);
    uecm_byte_writer_append_bytes(stream, iv, (size_t)iv_len);
    uecm_byte_writer_append_bytes(stream, cipher_data_temp, (size_t)cipher_data_len_temp);
    if (signature) {
        uecm_byte_writer_append_bytes(stream, signature, signature_size);
    }

    *cipher_data_size = uecm_byte_stream_get_size(stream);
    *cipher_data = uecm_bytes_create_from_bytes(uecm_byte_stream_get_data(stream), *cipher_data_size);

    result = true;

clean_up:
    uecm_safe_free(encrypted_key);
    uecm_safe_free(iv);
    uecm_safe_free(cipher_data_temp);
    uecm_byte_stream_destroy(stream);
    /* @todo fix seg fault if uncomment this following line */
    //uecm_safe_free(signature);
    uecm_signer_destroy(signer);
    uecm_safe_free(compressed);
    return result;
}

bool uecm_decipher_cipher_data(unsigned char *cipher_data,
    size_t cipher_data_size, uecm_private_key *private_key,
    uecm_public_key *public_key, unsigned char **plain_data,
    size_t *plain_data_size, const char *cipher_name,
    const char *digest_name) {

    bool result, verify_signature;
    uecm_byte_stream *stream;
    unsigned char *cipher_data_temp, *encrypted_key, *iv, *signature, *compressed;
    int cipher_data_len_temp, encrypted_key_len, iv_len, signature_size, plain_data_size_read, compressed_size;
    uecm_signer *signer;

    result = false;
    stream = uecm_byte_stream_create();
    cipher_data_temp = NULL;
    encrypted_key = NULL;
    iv = NULL;
    signature = NULL;
    signature_size = 0;
    verify_signature = false;
    signer = NULL;
    compressed = NULL;

    uecm_byte_writer_append_bytes(stream, cipher_data, cipher_data_size);
    uecm_byte_stream_set_position(stream, 0);

    uecm_byte_read_next_int(stream, &encrypted_key_len);
    uecm_byte_read_next_int(stream, &iv_len);
    uecm_byte_read_next_int(stream, &cipher_data_len_temp);
    uecm_byte_read_next_int(stream, &signature_size);
    uecm_byte_read_next_int(stream, &plain_data_size_read);

    if (signature_size == 0 && public_key != NULL) {
        ei_stacktrace_push_msg("A public key is specified to verify the signature of the data, but the signature size is equal to 0");
        goto clean_up;
    } else if (signature_size > 0 && public_key == NULL) {
        ei_stacktrace_push_msg("A signature is specified in the data, but no public key is specified");
        goto clean_up;
    } else if (signature_size > 0 && public_key != NULL) {
        verify_signature = true;
    }

    uecm_byte_read_next_bytes(stream, &encrypted_key, (size_t)encrypted_key_len);
    uecm_byte_read_next_bytes(stream, &iv, (size_t)iv_len);
    uecm_byte_read_next_bytes(stream, &cipher_data_temp, (size_t)cipher_data_len_temp);

    if (verify_signature && !uecm_byte_read_next_bytes(stream, &signature, signature_size)) {
		ei_stacktrace_push_msg("Failed to read signature field");
		goto clean_up;
	}

    if (!envelope_open_buffer(uecm_private_key_get_impl(private_key),
        cipher_data_temp, cipher_data_len_temp, encrypted_key,
        encrypted_key_len, iv, &compressed, &compressed_size, cipher_name)) {

        ei_stacktrace_push_msg("Failed to open envelope buffer");
        goto clean_up;
    }

    *plain_data_size = plain_data_size_read;

    if (!(*plain_data = uecm_decompress_buf(compressed, (size_t)compressed_size, plain_data_size_read))) {
		ei_stacktrace_push_msg("Failed to decompress ServerHeader content");
		goto clean_up;
	}

    if (verify_signature) {
        if (!(signer = uecm_rsa_signer_create(public_key, private_key, digest_name))) {
            ei_stacktrace_push_msg("Failed to create signer to verify signature");
            goto clean_up;
        }
    	if (!uecm_signer_verify_buffer(signer, *plain_data, *plain_data_size, signature, signature_size)) {
            uecm_safe_free(*plain_data);
            *plain_data_size = 0;
    		ei_stacktrace_push_msg("Failed to verify the signature of the sender");
    		goto clean_up;
    	}
    }

    result = true;

clean_up:
    uecm_byte_stream_destroy(stream);
    uecm_safe_free(cipher_data_temp);
    uecm_safe_free(encrypted_key);
    uecm_safe_free(iv);
    uecm_safe_free(compressed);
    uecm_safe_free(signature);
    uecm_signer_destroy(signer);
    return result;
}
