...
static bool uecm_rsa_get_string_from_keypair(RSA *keypair, char **pub_key, char **priv_key, size_t *pub_key_length, size_t *priv_key_length) {
    ...
    uecm_safe_alloc(priv_key_tmp, char, priv_key_length_tmp + 1);
    uecm_safe_alloc(pub_key_tmp, char, pub_key_length_tmp + 1);
...
clean_up:
	uecm_safe_free(pub_key_buf);
	uecm_safe_free(priv_key_buf);
